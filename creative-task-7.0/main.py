...
class NeuralNetwork(object):
    def __init__(self, numInputs, numNeuronsByLayer):
        self.numInputs = numInputs
        self.numNeuronsByLayer = numNeuronsByLayer
        self.inputLayer = [ ]
        self.neurons = [ ]
    ...
    def initializeNeurons(self):
        ...
        for layerIndex in range(len(self.numNeuronsByLayer)):
            numNeuronsInLayer = self.numNeuronsByLayer[layerIndex]
            layer = [ ]
            if (layerIndex == 0):
                inputLayer = self.inputLayer
            else:
                inputLayer = self.neurons[layerIndex - 1]

            for i in range(numNeuronsInLayer):
                layer.append(Neuron(inputLayer))

            self.neurons.append(layer)

        # Define an output neuron that takes the last layer as its inputNeurons.
        self.outputNeuron = Neuron(self.neurons[len(self.neurons) - 1],
                                  useRelu=False)

    def getMeanSquaredError(self):
        numPoints = len(app.data) + 1

        # Calculates the Mean Squared Error.
        sumSquaredErrors = 0
        for datapoint in app.data:
            actualOutcome = datapoint[0]

            # This gets a list of all the inputs from index 1 to the end of the
            # datapoint.
            inputValue = datapoint[1:]
            predictedOutcome = self.makePrediction(inputValue)

            error = predictedOutcome - actualOutcome
            sumSquaredErrors += error ** 2

        meanSquaredError = sumSquaredErrors / numPoints
        return meanSquaredError

    def setInputNeurons(self, inputs):
        for index in range(self.numInputs):
            neuron = self.inputLayer[index]
            neuron.prediction = inputs[index]

    def makePrediction(self, inputs):
        self.setInputNeurons(inputs)

        for layer in self.neurons:
            for neuron in layer:
                neuron.makePrediction()

        self.outputNeuron.makePrediction()
        return self.outputNeuron.prediction

    def train(self):
        for layer in self.neurons:
            for neuron in layer:
                neuron.runGradientDescent()

        self.outputNeuron.runGradientDescent()

class Neuron(object):
    def __init__(self, inputNeurons, useRelu=True):
        self.numInputs = len(inputNeurons)
    ...
    def makePrediction(self):
        self.prediction = 0
        for index in range(self.numInputs):
            # Instead of this method being passed a list of inputs, the neuron
            # stores a list of Neurons, self.inputNeurons, whose predictions are
            # the inputs.

            # Gets the prediction from the neuron at the index in
            # self.inputNeurons.
            inputNeuron = self.inputNeurons[index]
            self.prediction += self.weights[index] * inputNeuron.prediction

        self.prediction += self.weights[self.numInputs]
    ...
    def runGradientDescent(self):
        currentError = network.getMeanSquaredError()

        for index in range(self.numInputs + 1):
            oldWeight = self.weights[index]
            weightChange = randrange(-100, 101) / 100

            self.weights[index] = oldWeight + weightChange
            newError = network.getMeanSquaredError()
            if (newError < currentError):
                currentError = newError
            else:
                self.weights[index] = oldWeight
...
def updatePlot():
    ...
    for x in range(0, 500, 5):
        xData.append(x)
        yData.append(network.makePrediction([ x ]))

    app.networkPlot.updateData(newXData=xData, newYData=yData, resizeRanges=False)
...
def onStep():
    if (network.getMeanSquaredError() > 10):
        network.train()
        updatePlot()
...
import csv
def import_data():
    # Extracts the btc price from the last 100 days from btc-usd.csv
    app.data = []
    day = 1
    with open("BTC-USD.csv", encoding="utf-8") as file:
